...
def fuzzy_match(name, choices, cutoff=0.6):
    if not name:
        return None
    name_clean = re.sub(r'[^\w\u4e00-\u9fa5]', '', name).strip()
    if name_clean in choices:
        return name_clean
    matches = difflib.get_close_matches(name, choices, n=1, cutoff=cutoff)
    if matches:
        return matches[0]
    matches_lower = difflib.get_close_matches(name.lower(), [c.lower() for c in choices], n=1, cutoff=cutoff)
    if matches_lower:
        idx = [c.lower() for c in choices].index(matches_lower[0])
        return choices[idx]
    return None
...
with st.expander("📋 输入比赛信息", expanded=True):
    c1, c2 = st.columns(2)
    with c1:
        home_input = st.text_input("主队", placeholder="支持模糊匹配（如：巴、brazil）")
        home = fuzzy_match(home_input, TEAM_NAMES) if home_input else None
        if home_input and not home:
            st.warning(f"未找到匹配球队，请检查输入。")
    with c2:
        away_input = st.text_input("客队", placeholder="支持模糊匹配")
        away = fuzzy_match(away_input, TEAM_NAMES) if away_input else None
        if away_input and not away:
            st.warning(f"未找到匹配球队，请检查输入。")
    if home and away:
        st.success(f"✅ 匹配成功：{home} vs {away}")
...
if st.button("🔮 开始推演", use_container_width=True):
    if not home or not away:
        ...
        st.bar_chart(chart_data.set_index("球队"))
...
客队：{away}（ELO={ae}, xG={ax}）
比赛阶段：{match_type}
时间：{match_dt.strftime('%Y-%m-%d %H:%M')}

【机器学习预测】
首推：{ml['result'] if ml else 'N/A'}（概率 {ml['result_prob']:.1%}）
总进球：{ml['goals'] if ml else 'N/A'}球

【四层推演】
胜平负首推：{res['direction_primary']}
比分首推：{res['score_primary']}
总进球首推：{res['goal_primary']}球

【卦象】
本卦：{gi['base_name']}
体用生克：{gi['ti_yong']}
卦象方向：{gua_direction}
共振：{'✅ 共振' if quant_direction == gua_direction else '⚠️ 分歧'}

【泊松分布】
预期进球：主 {lh:.2f} / 客 {la:.2f}
最可能比分：{sorted_scores[0][0]}（{sorted_scores[0][1]:.1%}）
================================
"""
        st.download_button("📋 导出报告", data=report_text, file_name=f"预测报告_{home}_vs_{away}.txt", mime="text/plain")
        ...
        st.session_state.predict_history.append(record)
