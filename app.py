...
def predict_match(home, away, match_type, home_elo, away_elo, home_xg, away_xg, home_form, away_form, patches):
    ...
    direction = "主胜" if home_win_prob > draw_prob and home_win_prob > away_win_prob else "客胜" if away_win_prob > draw_prob else "平局"
...
if st.button("🔮 开始推演", use_container_width=True):
    if not home or not away:
        ...
        # 根据比赛性质调整
        if match_type == "淘汰赛":
            match_type_key = "knockout"
        elif match_type == "保级/出线生死战":
            match_type_key = "draw"
        elif match_type == "强弱悬殊":
            match_type_key = "slaughter"
        else:
            match_type_key = "general"

        # 执行预测
        result = predict_match(
            home, away, match_type_key,
            home_elo, away_elo,
            home_xg, away_xg,
            home_form, away_form,
            patches
